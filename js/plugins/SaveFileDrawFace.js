...
// ----------------------------------------------------------------------------
// Version
// 1.1.1改 2017/03/28 難易度を表示する機能を追加
...
(function() {
    ...
    param.DifficultyLevel = getParamBoolean(['DifficultyLevel', '変数表示']);
    ...
    if (param.showMapName) {
        ...
        DataManager.makeSavefileInfo      = function() {
            ...
            info.difficultylevel = $gameVariables.value(4)
            return info;
        };
    }
    ...
    //難易度
    Window_SavefileList.prototype.drawDifficultyLevel = function(info, x, y, width) {
        if (!info.difficultylevel) return;
        this.drawText(info.difficultylevel, x, y, width, param.hiddenGameTitle ? 'left' : 'right');
    };
    ...
    Window_SavefileList.prototype.drawContents = function(info, rect, valid) {
        if (valid) {
            ...
            if(param.DifficultyLevel){
                if(info.difficultylevel == "EASY"){
                    this.changeTextColor(this.textColor(6))
                };
                if(info.difficultylevel == "NORMAL"){
                    this.changeTextColor(this.textColor(29))
                };
                if(info.difficultylevel == "INFERNO"){
                    this.changeTextColor(this.textColor(27))
                };
                
                this.drawDifficultyLevel(info, rect.x + 670, rect.y, rect.width - 192);
